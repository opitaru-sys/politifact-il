...
6. **תיאור אירוע חדשותי שקרה (לא טענה של הפוליטיקאי)** — דגל חשוב במיוחד. הפוליטיקאי מספר על אירוע שהתרחש (רצח, פיגוע, תאונה, החלטה משפטית, פעולת משטרה). הציטוט תוכנו פרשני-חדשותי, לא טענה אישית של הפוליטיקאי שדיוקו על המדף. בדיקת האם האירוע אכן קרה אינה בדיקת דיוק של הפוליטיקאי.
   דוגמאות לדחייה:
...
8. **טריוויה על אנשים שאינם פוליטיקאים** — דגל חשוב במיוחד. בדיקת עובדות ביוגרפיות על דמויות שאינן פוליטיקאיות (קורבנות אלימות, ספורטאים, אמנים, אקדמאים, אזרחים פרטיים). גם אם נכון, לא רלוונטי לדיוק הפוליטיקאי שאמר את הציטוט.
   דוגמאות לדחייה:
...
12. **מבחן עניין ציבורי** — דגל כללי שמכסה מקרים שהקטגוריות הקודמות לא תופסות. **טענה צריכה להתפרסם בפיד הציבורי רק אם אזרח/ית סבירים יחשבו שאכפת להם אם היא נכונה,** מפני שהיא משפיעה על: מדיניות, כספי ציבור, חקיקה, ביטחון, זכויות אזרח, מינויים, שחיתות, ביצועי תפקיד ציבורי, או אחריותיות מוסדית.

   **דחה אם הטענה היא בעיקר:**
   - שאיפה אישית או בקשת תפקיד ("ביקשתי להיות סגן רה"מ", "אני רוצה להיות שר X")
   - רכילות קואליציונית פרטית ("X אמר לי במו"מ", "Y הבטיח לי", "שרה נתניהו קפצה")
   - תיאור שיחה פרטית שאי-אפשר לאמת באופן עצמאי
   - תמרון פוליטי פנימי ללא השלכה ציבורית
   - פלאף ביוגרפי או אנקדוטלי

   **חריג חשוב — אשר למרות התבנית למעלה:** אם הפרט הפרטי/קואליציוני חושף **השפעה משמעותית** על אחד מאלה:
   - מינויים ציבוריים
   - מדיניות, כסף ציבורי, או חקיקה
   - חוקיות, שחיתות, או אחריותיות
   - סמכות חיצונית (דתית/עסקית/זרה) המנחה החלטות ממשלה

   **דוגמאות:**
   - "ביקשתי להיות סגן רה"מ" → דחה. שאיפת תפקיד אישית ללא תוכן.
   - "בסוף ביבי אמר שהוא ייתן לי שר כפול" → דחה. דיווח על שיחה פרטית.
   - "הרבי מגור הורה לנו לדרוש את תפקיד סגן רה"מ" → **אשר.** חושף השפעה דתית חיצונית על מינויים ממשלתיים. החריג חל.
   - "השר X דרש 50 מיליון נוספים לתקציב Y" → אשר. השפעה על כספי ציבור.
   - "פגשתי את ראש הסי-איי-איי" → דחה. ביוגרפי, ללא תוכן מדיניות.

   הערה: דחייה כאן אינה "פסילה לצמיתות" — זו הסתרה מהפיד הציבורי. כשהמודל הארכיטקטוני של שלוש קטגוריות (public / profile_only / rejected) יוטמע, טענות שדחיתי כאן יישארו זמינות בעמוד הפוליטיקאי בקטגוריה profile_only. בינתיים, הסתרה היא הצעד הנכון.
...
5. **טענה שאישורה או הפרכתה משקפת על דיוק הפוליטיקאי על נושאי מדיניות מהותיים.** לא "מידע חדש על העולם בכלל", אלא "מידע חדש על מה שהפוליטיקאי טוען בנוגע למדיניות".

**הסטנדרטים — שונים לפי קטגוריה:**
- בספק לגבי קטגוריות 1-5 (PR / ועדה / תודות / אישי / טריוויאל): **אשר.** דחה רק כשברור.
- בספק לגבי קטגוריות 6-8 (אירוע חדשותי / ארגוני / טריוויה על לא-פוליטיקאי): **דחה.** הקטגוריות האלה נוטות לדלל את מדד הדיוק עם תוכן שאינו טענה אישית של הפוליטיקאי. דחה אלא אם הציטוט באמת מכיל טענה מהותית של הפוליטיקאי על מדיניות.
- בספק לגבי קטגוריות 9-11 (שיפוט retroactive / טענה אפיונית / הצהרת כוונה מוסדית): **דחה.** אלה פגמים יסודיים בדיוק הבדיקה עצמה — verdict שגוי שמתפרסם בפומבי. עדיף להוריד טענה ספציפית מאשר לתת לקורא בדיקה שאינה משקפת את האמת.
- בספק לגבי קטגוריה 12 (מבחן עניין ציבורי): **דחה.** אם אי-אפשר להסביר בכנות למה אזרח סביר אכפת לו אם הטענה נכונה, היא לא צריכה להופיע בפיד הציבורי. החריג של "חושף השפעה משמעותית" צריך להיות ברור ומוכח, לא נדחק בלחץ.
